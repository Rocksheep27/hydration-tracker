...
推荐优先使用 **GitHub Pages**，采用一个独立的公开发布仓库，只包含经过检查的 PWA 静态文件，不上传当前完整项目目录。
...
- 首次本地提交已完成，但尚未执行 `git push`，也未开启 GitHub Pages。
...
GitHub Pages 的分支发布来源只支持分支根目录或 `/docs`，不能直接选择当前的 `pwa/`。建议后续增加一个最小 GitHub Actions 工作流，把 `pwa/` 作为 Pages artifact 上传并部署。该工作流尚未创建，也未执行发布。
...
- 本轮禁止推送。用户明确说“可以推送”后，才允许执行 `git push -u origin main`。
- 首次推送完成后，再单独增加 Pages 工作流并在 GitHub 设置中选择 GitHub Actions 作为发布来源。
